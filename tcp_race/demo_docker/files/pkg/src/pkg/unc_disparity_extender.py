...
import numpy as np
import pkg.mockrospy as mockrospy
#from driver import Driver
#from drivers import Driver

#from map_gym import make_map
...
class DisparityExtenderDriving:
    ...
    def __init__(self, tuning_gains=False, filename=None):
        ...
        if filename is None:
            filename = "gains_unc.pkl"

        # filename for gain lookup
        ...
        self.max_speed = 7.75 # 7.75 is good
        self.absolute_max_speed = 20  #self.max_speed # 0.4
        ...
        self.pub_drive_param = mockrospy.Publisher('drive_parameters',
            my_drive_params, queue_size=5)
    ...
    def crashed(self):
        ...
        if last_pos != DisparityExtenderDriving.LAST_CRASH_POS:
            DisparityExtenderDriving.LAST_CRASH_POS = last_pos
            DisparityExtenderDriving.SAME_POS_CRASH_COUNT = 1

            print(f"Crash was in a new location: {last_pos}")
        else:
            DisparityExtenderDriving.SAME_POS_CRASH_COUNT += 1
            count = DisparityExtenderDriving.SAME_POS_CRASH_COUNT
        ...
        # save to file
        parent = os.path.dirname(os.path.realpath(__file__))
        path = os.path.join(parent, 'gains_unc.pkl')
    ...
    def update_params(self, ego_odom):
        """update gains based on current position"""

        x = int(round(ego_odom['pose_x'])) // 20
        y = int(round(ego_odom['pose_y'])) // 20
        pt = (x, y)
        first = False
        ...
        if not self.pos_history:
            ...
            if pt in self.gain_dict:
                self.absolute_max_speed = self.gain_dict[pt]
        ...
        if pt != self.pos_history[-1] or first:
            ...
            if pt not in self.gain_dict:
                cur_gain = 20 if self.tuning_gains else None #7.75
            else:
        ...
        #if self.absolute_max_speed > self.target_gain:
        #    self.absolute_max_speed -= 0.01
        #elif self.absolute_max_speed < self.target_gain:
        #    self.absolute_max_speed += 0.005
        self.absolute_max_speed = self.target_gain

    def run(self):
        """ Starts running the car until we want it to stop! """
        
        start_time = time.time()
        mockrospy.Subscriber('scan', MyLaserScan, self.lidar_callback)
        mockrospy.spin()
...
class DisparityExtenderUNCDriver:
    """Driver for smooth blocking planner"""

    def __init__(self, tuning_gains=False, filename=None):
        mockrospy.topics_sub = {} # clear pub/sub topics
        
        self.planner = DisparityExtenderDriving(tuning_gains=tuning_gains, filename=filename)

        self.driving_params = my_drive_params(0, 0)
        
        self.pub = mockrospy.Publisher('scan', MyLaserScan, queue_size=5)
        self.sub = mockrospy.Subscriber('drive_parameters', my_drive_params, self.driving_params_callback)
        ...
        self.topics_sub = mockrospy.topics_sub
        mockrospy.topics_sub = None
    ...
    def load_gain_dict(self):
        ...
        self.planner.load_gain_dict()

    def crashed(self):
        """car crasehd while tuning"""

        self.planner.crashed()
    ...
    #def plan(self, obs, ego_index):
        ...
        #self.planner.absolute_max_speed = 7.75
        self.planner.update_params(ego_odom)

        # use appropriape pubsub
        mockrospy.topics_sub = self.topics_sub

        #self.pub.publish(LaserScan(obs["scans"][ego_index].tolist()))
        if isinstance(ranges, tuple):
            self.pub.publish(MyLaserScan(list(ranges)))
        else:
            self.pub.publish(MyLaserScan(ranges.tolist()))
