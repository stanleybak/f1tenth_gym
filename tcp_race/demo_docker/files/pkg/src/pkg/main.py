...
from pkg.switching import SwitchingDriver
...
# switching in front
ego_index = 0
drivers = [SwitchingDriver()]
#drivers = [SwitchingDriver(), MyGapFollower()]

# switching behind
#ego_index = 1
#drivers = [MyGapFollower(), SwitchingDriver()]

render_mode = 'human_fast' # 'human_fast' or 'human'
...
class GymRunner(object):
    ...
    def run(self):
        ...
        if crashed:
            #de.planner.crashed() # updats gains for next try
            print(f"CRASHED! collisons: {obs['collisions']}")
...
def render_callback(env_renderer):
    ...
    index = ego_index # index of car to follow

    # update camera to follow car
    x = e.cars[index].vertices[::2]
    y = e.cars[index].vertices[1::2]
