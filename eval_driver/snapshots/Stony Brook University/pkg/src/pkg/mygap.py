...
class MyGapFollower:
    ...
    def __init__(self, tuning_gains=False, filename=None):
        # used when calculating the angles of the LiDAR data
        self.radians_per_elem = None

        self.filename = 'gains_gap.pkl' if filename is None else filename
        ...
        self.slow_steer_count = 0

    def load_gain_dict(self):
        """load gain dict from file"""

        parent = os.path.dirname(os.path.realpath(__file__))
        path = os.path.join(parent, self.filename)
        ...
        try:
            ...
            assert self.tuning_gains, f"{path} not found"
    ...
    def crashed(self):
        ...
        print(f"history len: {len(self.pos_history)}, max_history: {self.max_history}")
        self.max_history = max(self.max_history, len(self.pos_history))
        ...
        for i in range(count):
            if index - i < 0: # self.max_history??
                break

            should_break = False
            
            for checkpoint in [12, 27]:
                if index - i < checkpoint and self.max_history > checkpoint:
                    should_break = True
                    print("crasehd before checkpoint?!?")
                    break
            ...
            new_gain = (2*self.gain_dict[pos] + min_gain) / 3
    ...
    def update_params(self, ego_odom):
        ...
        if pt != self.pos_history[-1] or first:
            ...
            if pt not in self.gain_dict:
                cur_gain = 20 if self.tuning_gains else None
            else:
    ...
    def get_angle(self, range_index, range_len):
        ...
        if self.slow_steer_count > 0:
            self.slow_steer_count -= 1
